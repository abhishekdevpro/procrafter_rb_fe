...
const Testpaper = () => {
  ...
  const handleBack = () => {
    router.back('skilltest');
  };

  if (loading) {
    return (
      <div className="fixed inset-0 bg-violet-950 bg-opacity-10 backdrop-blur-sm flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-xl flex flex-col items-center space-y-4">
          <div className="w-16 h-16 border-4 border-violet-950 border-t-transparent rounded-full animate-spin"></div>
          <h2 className="text-xl font-bold text-violet-950">Please Wait</h2>
          <p className="text-sm text-red-500 font-medium">Loading your test questions...</p>
          <p className="text-xs text-gray-500">Do not close or refresh this window</p>
        </div>
      </div>
    );
  }

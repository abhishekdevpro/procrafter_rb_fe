...
const PersonalInformation = () => {
  ...
  const { resumeData, setResumeData, handleProfilePicture, handleChange, resumeStrength, setResumeStrength } =
    useContext(ResumeContext)
  const router = useRouter()
  const { improve } = router.query
  ...
  useEffect(() => {
    const fetchCountryCodes = async () => {
      setIsLoading((prev) => ({ ...prev, countryCodes: true }))
      try {
        const response = await fetch(`${BASE_URL}/api/user/countries?lang=${language}`)
        if (response.ok) {
          const data = await response.json()
  ...
  const fetchJobTitles = async (keyword) => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/api/user/job-title?job_title_keyword=${encodeURIComponent(keyword)}&lang=${language}`,
      )
      if (response.ok) {
        const data = await response.json()
        const jobTitles = data.data.map((item) => item.name)
  ...
  const fetchLocations = async (keyword) => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/api/user/locations?locations=${encodeURIComponent(keyword)}&lang=${language}`,
      )
      if (response.ok) {
        const data = await response.json()
        const locations = data.data.location_names.map((item) => item)
  ...
  const selectCountryCode = (country) => {
    ...
    // Update contact information with new country code
    if (resumeData.contactInformation) {
      const contactWithoutCode = resumeData.contactInformation.replace(/^(\+\d+\s*)?/, "")
      ...
      const validationError = validateField("contactInformation", `${newCountryCode} ${limitedContact}`)
      setValidationErrors((prev) => ({
        ...prev,
  ...
  const handleContactChange = (e) => {
    ...
    const validationError = validateField("contactInformation", fullContactValue)
    setValidationErrors((prev) => ({
      ...prev,
      contactInformation: validationError,
    }))
  ...
  return (
    <div className="flex flex-col gap-3 w-full items-center md:mt-10 p-4 md:px-10">
      <h2 className="text-2xl md:text-3xl font-semibold text-black">{t("builder_forms.personal_info.details_info")}</h2>
                      ...
                      <div className="flex items-center space-x-2">
                        {(field === "name" || field === "position" || field === "contactInformation") && (
                          <button
                            ...
                            disabled={isLoading.autoFix || !resumeData[field]?.trim()}
                            className="px-3 py-1 text-sm font-medium text-white bg-blue-600 rounded-md shadow hover:bg-blue-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            {isLoading.autoFix ? <Loader2 className="w-4 h-4 animate-spin" /> : "Auto Fix"}
                          </button>
                        )}
